...
class ShimConcurrentPool {
  ...
  private:
    // NOT IMPLEMENTED
    ShimConcurrentPool(const ShimConcurrentPool&);
...
// FREE OPERATORS
void *operator new(bsl::size_t size, BloombergLP::bdlma::ShimConcurrentPool& pool);
    ...
    // argument:
    //..
    //  my_Type *newMyType(bdlma::ShimConcurrentPool *pool,
    ...
    //..
    //  void deleteMyType(my_Type *t, bdlma::ShimConcurrentPool *pool)
    //  {
    //      pool->deleteObject(t);
    //  }
    //..
...
void operator delete(void *address, BloombergLP::bdlma::ShimConcurrentPool& pool);
    ...
    // 'bdlma::ShimConcurrentPool::deleteObject()' instead.
...
template<class TYPE>
inline
void ShimConcurrentPool::deleteObject(const TYPE *object)
{
    if (nullptr != object)
    {
...
inline
void ShimConcurrentPool::release()
{
    d_mutex.lock();
    pool.release();
    d_mutex.unlock();
...
// ACCESSORS
inline
bsls::Types::size_type ShimConcurrentPool::blockSize() const
{
    return d_blockSize;
}
...
inline
bslma::Allocator *ShimConcurrentPool::allocator() const
{
