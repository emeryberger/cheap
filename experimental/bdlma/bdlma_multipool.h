...
#ifndef BDE_USE_ORIGINAL_MULTIPOOL
#error "Missing definition."
#endif

#if !BDE_USE_ORIGINAL_MULTIPOOL
#include "cheap/experimental/shim_multipool.hpp"
#endif
...
//
//@CLASSES:
//  bdlma::Multipool_REAL: memory manager that manages pools of varying block sizes
//
//@SEE_ALSO: bdlma_pool, bdlma_multipoolallocator
//
//@DESCRIPTION: This component implements a memory manager, 'bdlma::Multipool_REAL',
...
// 'bdlma::Multipool_REAL' release all memory currently allocated via the object.
//
// A 'bdlma::Multipool_REAL' can be depicted visually:
//..
...
///-----------------------------
// When creating a 'bdlma::Multipool_REAL', clients can optionally configure:
//
...
// currently installed default allocator at the time the 'bdlma::Multipool_REAL' was
// created.
//
...
//
///Example 1: Using a 'bdlma::Multipool_REAL' Directly
/// - - - - - - - - - - - - - - - - - - - - - - -
// A 'bdlma::Multipool_REAL' can be used by containers that hold different types of
...
// messages, we use a 'bdlma::Multipool_REAL'.
//
...
//
//      // DATA
//      bdlma::Multipool_REAL d_multipool;  // multipool used to supply memory
//
//    private:
//      // Not implemented:
...
// the messages using memory provided by the multipool.  A 'bdlma::Multipool_REAL'
...
//..
//
///Example 2: Implementing an Allocator Using 'bdlma::Multipool_REAL'
/// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
// 'bdlma::Pool' objects in a similar fashion to 'bdlma::Multipool_REAL'.  In this
// example, we create just such a multipool allocator, 'my_Multipool_REALAllocator',
// that uses a 'bdlma::Multipool_REAL' to manage the multiple pools.
//
// First, we define the interface of 'my_Multipool_REALAllocator':
//..
//  class my_Multipool_REALAllocator : public bslma::Allocator {
...
//
//      // DATA
//      bdlma::Multipool_REAL d_multiPool;  // memory manager for allocated memory
//                                     // blocks
//
//    public:
//      // CREATORS
//      my_Multipool_REALAllocator(bslma::Allocator *basicAllocator = 0);
...
//      // ...
//
//      virtual ~my_Multipool_REALAllocator();
...
//
// Finally, we provide the trivial implementation of 'my_Multipool_REALAllocator':
//..
//  // CREATORS
//  inline
//  my_Multipool_REALAllocator::my_Multipool_REALAllocator(
...
//  // MANIPULATORS
//  inline
//  void *my_Multipool_REALAllocator::allocate(bsls::Types::size_type size)
//  {
//      if (0 == size) {
//          return 0;                                                 // RETURN
//      }
...
//
//  inline
//  void my_Multipool_REALAllocator::deallocate(void *address)
//  {
...
class Multipool_REAL {
    ...
    // 'bdlma::Multipool_REAL' release all memory currently allocated via the
    // object.
  ...
  private:
    // NOT IMPLEMENTED
    Multipool_REAL(const Multipool_REAL&);
    Multipool_REAL& operator=(const Multipool_REAL&);

  public:
    // CREATORS
    explicit
    Multipool_REAL(bslma::Allocator *basicAllocator = 0);
    explicit
    Multipool_REAL(int numPools, bslma::Allocator *basicAllocator = 0);
    explicit
    Multipool_REAL(bsls::BlockGrowth::Strategy  growthStrategy,
              bslma::Allocator            *basicAllocator = 0);
    Multipool_REAL(int                          numPools,
...
template <class TYPE>
inline
void Multipool_REAL::deleteObject(const TYPE *object)
{
...
template <class TYPE>
inline
void Multipool_REAL::deleteObjectRaw(const TYPE *object)
{
...
// ACCESSORS
inline
int Multipool_REAL::numPools() const
{
    return d_numPools;
}

inline
bsls::Types::size_type Multipool_REAL::maxPooledBlockSize() const
{
...
inline
bslma::Allocator *Multipool_REAL::allocator() const
{
    return d_allocator_p;
}

#if BDE_USE_ORIGINAL_MULTIPOOL
#warning "USING ORIGINAL MULTIPOOL"
using Multipool = Multipool_REAL;
#else
using Multipool = ShimMultipool;
#endif
