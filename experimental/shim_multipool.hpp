...
class ShimMultipool {
  ...
  private:
    // NOT IMPLEMENTED
    ShimMultipool(const ShimMultipool&);
    ShimMultipool& operator=(const ShimMultipool&);
  ...
  public:
    ...
    {}
    explicit
    ShimMultipool(bsls::BlockGrowth::Strategy  growthStrategy,
...
template <class TYPE>
inline
void ShimMultipool::deleteObject(const TYPE *object)
{
    if (nullptr != object)
    {
...
template <class TYPE>
inline
void ShimMultipool::deleteObjectRaw(const TYPE *object)
{
    if (nullptr != object)
    {
...
// ACCESSORS
inline
int ShimMultipool::numPools() const
{
    return d_numPools;
}
...
inline
bslma::Allocator *ShimMultipool::allocator() const
{
